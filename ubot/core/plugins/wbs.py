...
async def take_ss(_, message):
    if len(message.command) < 2:
        return await message.reply("<code>Berikan saya link yang valid</code>")
    ...
    if len(message.command) == 2:
        ...
        return await message.reply("<code>Ada yang salah.</code>")

    m = await message.reply("<code>Processing...</code>")
